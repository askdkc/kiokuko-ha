...
**雑談を含む、認証済みの全完了ターンで追加AI呼び出しが発生します。** 有用な経験がなければ保存しません。抽出は非同期・profileごとに同時1件です。入力は最大32,000文字の重なり付き窓へ分け、1 runにつき4窓まで処理します。待機上限はモデル呼び出しごとに12秒です。窓進捗を保存して再開し、対象外の範囲は`monitor show`へ表示します。時間切れや記録欠落を成功扱いにしません。
...
CLI・DMは本人とworkspace、groupは会話とworkspaceの境界で保存します。workspaceがなければ本人または会話だけに限定します。profile全体には自動共有しません。承認済み・ファイル検証済み記憶を優先し、経験は「未検証の過去事例」として、教訓と合わせて最大2件・800文字、既存の総上限2,200文字内で注入します。

経験の有効期間は観測したrunの完了から90日です。同じscopeの独立した新しい観測は期間を延長できます。再処理・再生成・再検索では期間を増やしません。教訓の学習は既定`off`の別設定です。[off/shadow/auto、根拠の条件、品質評価](learning.md)を参照してください。
...
前者は元traceを削除して抽出jobを止め、その未検証経験を失効させ、依存する自動教訓と旧自動履歴を削除します。後者は対象記憶と根拠、依存する自動教訓を削除し、その元runのjobを止めます。通常の保持期限による削除では照合済みの最小根拠を残しますが、明示的purgeでは自動教訓を残しません。人間が訂正した現本文は保持し、旧自動教訓履歴を削除します。本文を含まない処理receiptを残し、再処理による復活を防ぎます。Hermesの履歴や既存backup/exportは別です。
