...
Every completed, authenticated turn schedules asynchronous extraction, including ordinary conversation. The model may return no experience. **These are additional model calls**, separate from the main conversation and existing compaction. Long input uses overlapping, addressed windows of at most 32,000 characters, with at most four windows per run and a 12-second wait per model call. Validated window progress is resumable; excluded ranges are reported by `monitor show`. Failed/oversized/missing captures are visible and are not silently treated as complete.
...
CLI/DM experiences belong to the principal and workspace (principal only if no workspace). Group experiences belong to the conversation and workspace (conversation only if no workspace). No automatic profile-wide sharing occurs. Existing approved/file-verified memory has priority. Recall includes at most two experiences and lessons combined / 800 characters within the existing 2,200-character total budget, explicitly labeled as unverified historical examples. Recalled content does not count as independent corroboration. Independent new runs may renew an active experience's lifetime to 90 days after the observed run completed; replay, regeneration and recall do not. Optional derived lessons have separate off/shadow/auto controls, default off; see [learning, evidence gates and evaluation](learning.md).
...
The first command removes the source trace, blocks extraction, expires its unverified experiences, and deletes dependent automatic lessons and their automatic history. The second deletes the selected memory and evidence, removes dependent automatic lessons, blocks its source jobs, and keeps content-free receipts to prevent regeneration by reprocessing. Normal retention cleanup preserves previously verified minimal evidence; explicit purge does not preserve automatic lessons. Human-corrected current text is retained, while its old automatic lesson history is removed. Neither deletes Hermes history or existing backups/exports. Revision, correction, forget and expiry use the existing memory lifecycle.
